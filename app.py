...
# VERSION 9 – SMOOTH PARETO CURVE + HARD LIMITS
# ================================================================
...
# ================================================================
# RESULT FUNCTIONS (filter feasible)
# ================================================================
...
def run_real_optimization(progress_callback=None):
    ...
    pareto_obj = final_obj[pareto_idx]  # NEW: keep objectives aligned with pareto_pop, row-for-row
    ...
    for i, (row, pred) in enumerate(zip(pareto_pop, preds)):
        ...
        # BUGFIX (root cause of the golden star floating off the Pareto
        # curve): this previously used `pred[2]` — a raw, unshrunk model
        # prediction from a fresh model.predict() call — for EFRF. But the
        # Pareto chart plots `pareto_objectives[:, 2]`, which is the
        # shrinkage-adjusted EFRF computed inside evaluate() (out-of-
        # distribution candidates get pulled toward the training mean;
        # see NSGAIIOptimizer.evaluate()). For the same formulation these
        # are two different numbers, so the "golden" point — built from
        # the raw value — didn't land on the curve built from the shrunk
        # values. Using the identical shrunk value here guarantees golden
        # is always exactly a point on the plotted front.
...
def render_pareto_evolution():
    ...
    # BUGFIX/IMPROVEMENT: the "feasible region" was previously ~3000
    # randomly-sampled Monte Carlo points scattered across the whole
    # plot (visible as noisy speckle in the reported screenshot) —
    # informative but not what was asked for ("a light color area").
    # Replaced with a clean shaded rectangle covering the actual
    # constraint region (EFRF < 0.40, within the API design bounds),
    # drawn as the bottom-most layer so every other trace draws on top
    # of it.
    fig.add_hrect(
    ...
    # add_hrect draws a shape, which Plotly does not add to the legend on
    # its own — this invisible proxy trace gives it a legend entry.
    ...
    # Boundaries
    # BUGFIX: all three of these previously used Plotly's default
    # annotation placement, which put the EFRF-limit label and the
    # API-max label in the same corner (top-right) — they rendered
    # stacked on top of each other, unreadable, as seen in the reported
    # screenshot. Each now gets an explicit, distinct corner.
